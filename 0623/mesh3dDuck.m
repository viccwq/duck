...
% depth_h=(depth_h-min(min(min(depth_h,[],3))))*40+4;%1 0.3
depth_h=depth_h/max(max(max(abs(depth_h),[],3)))*10+0.1;%1 0.3
% 显示等值面
% figure(2)
% isosurface(depth,0);
% axis equal
...
%rat2 0.2不行 0.25有点形态
%duck 0.3
temp=rand(size(p,1),1)<0.05;% 0.45
p=p(temp,:);
...
p=p(rand(size(p,1),1)<(min(r0)+0.1)^(4/1)./r0.^(4/1),1:3);
