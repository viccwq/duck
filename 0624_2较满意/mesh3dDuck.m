...
if swith
figure
% slice(permute(vol,[2,1,3]),[75,100],[100],[100]);
% shading flat
% colorbar
hold on
%三角形的重心
center=(node(element(:,1),:)+node(element(:,2),:)+node(element(:,3),:))/3;
% load depth_start;
load depth_h;
depth_h=smooth3(depth_h,'box',3);
depth_h=smooth3(depth_h,'box',7);
depth_h=smooth3(depth_h,'box',7);
[ temp,Val ] = pointSize1_N(depth_h,center,size(depth_h));
trisurf(element,node(:,1),node(:,2),node(:,3),Val,'edgecolor','k','EdgeAlpha',0.2);
% trimesh(element,node(:,1),node(:,2),node(:,3),'edgecolor','k','FaceAlpha',0.5,'FaceColor','w');
colormap;
xlabel ( '<--- X --->' );
ylabel ( '<--- Y --->' );
zlabel ( '<--- Z --->' );
...
[ outPoint,outCurve ] = pointCurve(node,element,vol);%计算曲率
...
[p,t]=distmeshnd_vic(fd,@huniform,mean(eledist),box,fix,outPoint,outCurve,vol);
...
%%
function [p,t]=distmeshnd_vic(fdist,fh,h,box,fix,node,curve,vol,varargin)
...
% 	[ fixInfor,tree ] = buildOctreeVic( box(:)',[node,ones(size(node,1),1)*7] ); 
	[ fixInfor,tree ] = buildOctreeVic( box(:)',[node,round(curve*10)] ); 
    eval([str,'=tree;']);
    save(str,str);
 end
eval(['clear ',str,';']);
...
depth_h=smooth3(depth_h,'box',7);
depth_h=smooth3(depth_h,'box',7);
depth_h=smooth3(depth_h,'box',9);
depth=depth/max(max(max(abs(depth),[],3)))*10;
depth_h=(depth_h-min(min(min(depth_h,[],3))))*2.0+1;%1 0.3
% depth_h=depth_h/max(max(max(abs(depth_h),[],3)))*5+0.00;%1 0.3
% 显示等值面
% figure(2)
% isosurface(depth,0);
% axis equal
...
%球区域外点的法向量
out_nodes=p(sizeVal<8&sizeVal>-8,:);
...
boundary=0.05;%0.2
p=p(sizeVal>boundary,:);%0.2

%平均概率筛选
%rat2 0.2不行 0.25有点形态
%duck 0.3
temp=rand(size(p,1),1)<0.45;% 0.45
p=p(temp,:);
% p=[p;p+repmat([2.4,0,0],size(p,1),1)];
p=[p;p+repmat([2.4,0,0],size(p,1),1);p+repmat([0,3.7,0],size(p,1),1);p+repmat([0,0,4.2],size(p,1),1)];
[ temp,sizeVal_h ] = pointSize1_N(depth_h,p,size(depth_h));

r0=sizeVal_h/max(sizeVal_h);
% p=p(rand(size(p,1),1)<(min(r0)+0.05)^(3/2)./r0.^(3/2),1:3);
p=p(rand(size(p,1),1)<(min(r0)+0.01)^(3/1)./r0.^(3/1),1:3);
